...
#include "string.h"
#include "assert.h"
#include "../kernel/arch/x86_64/cpu/control_regs_constants.h"
...
mtrr_info_t::mtrr_info_t()
{
    ...
    reg = cpu_msr_get(CPU_MTRR_DEF_TYPE);

    def_type = reg & CPU_MTRR_DEF_TYPE_MASK;
    enabled_fixed = support_fixed && (reg & CPU_MTRR_DEF_TYPE_FIXED_EN);
    enabled_mtrr = reg & CPU_MTRR_DEF_TYPE_MTRR_EN;

    if (enabled_fixed) {
        reg = cpu_msr_get(CPU_MTRR_FIX64K_00000);
        memcpy(fixed_types + 0 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX16K_80000);
        memcpy(fixed_types + 1 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX16K_A0000);
        memcpy(fixed_types + 2 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_C0000);
        memcpy(fixed_types + 3 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_C8000);
        memcpy(fixed_types + 4 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_D0000);
        memcpy(fixed_types + 5 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_D8000);
        memcpy(fixed_types + 6 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_E0000);
        memcpy(fixed_types + 7 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_E8000);
        memcpy(fixed_types + 8 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_F0000);
        memcpy(fixed_types + 9 * sizeof(reg), &reg, sizeof(reg));

        reg = cpu_msr_get(CPU_MTRR_FIX4K_F8000);
    ...
    if (vcnt) {
        ...
        for (size_t i = 0; i < vcnt; ++i) {
            fixed_bases[i] = cpu_msr_get(CPU_MTRR_PHYSBASE_n(i));
            fixed_masks[i] = cpu_msr_get(CPU_MTRR_PHYSMASK_n(i));
        }
    }
}

uint8_t mtrr_info_t::memtype_of(uint64_t addr)
{
    uint64_t byte_index;
