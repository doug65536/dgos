.macro gdt_seg priv, exec, rw, gran, is32, is64, base_md, limit_hi
    // limit 15:0
    .hword 0xFFFF
    // base 15:0
    .hword 0
...
.macro gdt_seg_code priv, gran, is32, is64, base_md, limit_hi
    gdt_seg \priv, 1, 1, \gran, \is32, \is64, \base_md, \limit_hi
.endm

.macro gdt_seg_data priv, gran, is32, base_md, limit_hi
    gdt_seg \priv, 0, 1, \gran, \is32, 0, \base_md, \limit_hi
.endm

.macro gdt_emit_desc sel
    .if \sel == GDT_SEL_KERNEL_CODE64
        gdt_seg_code 0,1,0,1,0,0xF

    .elseif \sel == GDT_SEL_KERNEL_DATA
        gdt_seg_data 0,1,1,0,0xF

    .elseif \sel == GDT_SEL_PM_CODE32
        gdt_seg_code 0,1,1,0,0,0xF

    .elseif \sel == GDT_SEL_PM_DATA32
        gdt_seg_data 0,1,1,0,0xF

    .elseif \sel == GDT_SEL_PM_CODE16
        gdt_seg_code 0,0,0,0,0,0x0

    .elseif \sel == GDT_SEL_PM_DATA16
        gdt_seg_data 0,0,0,0,0x0

    .elseif \sel == GDT_SEL_PM_STAC16
        // 16-bit PM stack base at 0x2000
        gdt_seg_data 0,0,0,0x2,0x0

    .elseif \sel == GDT_SEL_USER_CODE32
        gdt_seg_code 3,1,1,0,0,0xF

    .elseif \sel == GDT_SEL_USER_DATA
        gdt_seg_data 3,1,1,0,0xF

    .elseif \sel == GDT_SEL_USER_CODE64
        gdt_seg_code 3,1,0,1,0,0xF
...
.macro gdt_emit gdt_label gdtr_label
	...
	\gdtr_label :
		...
		gdt_emit_desc GDT_SEL_PM_STAC16
