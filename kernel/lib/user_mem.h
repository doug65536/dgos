...
_use_result
KERNEL_API bool mm_copy_user(void *dst, void const *src, size_t size);
...
_use_result
KERNEL_API ptrdiff_t mm_copy_user_str(char *dst, char const *src, size_t size);

KERNEL_API intptr_t mm_lenof_user_str(char const *src, size_t max_size);

int mm_compare_exchange_user(volatile int *dest, int *expect, int replacement);

_use_result
_const
KERNEL_API bool mm_is_user_range(void const *buf, size_t size);
