...
UNITTEST(test_bit_uint64_t)
{
    for (uint64_t i = 0; i < 63; ++i) {
        ...
        uint64_t h = bit_log2(n + ext::max(uint64_t(1), n - 1));
...
UNITTEST(test_bit_uint32_t)
{
    for (uint32_t i = 0; i < 31; ++i) {
        ...
        uint32_t h = bit_log2(n + ext::max(uint32_t(1), n - 1));
...
UNITTEST(test_bit_uint16_t)
{
    for (uint16_t i = 0; i < 15; ++i) {
        ...
        uint16_t h = bit_log2(n + ext::max(uint16_t(1), uint16_t(n - 1)));
...
UNITTEST(test_bit_uint8_t)
{
    for (uint8_t i = 0; i < 7; ++i) {
        ...
        uint8_t h = bit_log2(n + ext::max(uint8_t(1), uint8_t(n - 1)));
