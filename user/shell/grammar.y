...
declarations(A) ::= declarations(B) declaration(C) . { A = parser_node(2, Op_Declarations, B, C); }
declaration(A) ::= statement(B) . { A = B; }
statement(A) ::= assignment(B) . { A = B; }
assignment(A) ::= name(B) EQ expression(C) . { A = parser_node(2, Op_Assign, B, C); }
value(A) ::= name(B) . { A = parser_node(1, Op_Identifier, B); }
value(A) ::= number(B) . { A = parser_node(1, Op_Number, B); }
expression(A) ::= value(B) . { A = B; }
expression(A) ::= expression(B) PLUS value(C) . { A = parser_node(2, Op_Add, B, C); }
expression(A) ::= expression(B) MINUS value(C) . { A = parser_node(2, Op_Sub, B, C); }
expression(A) ::= expression(B) TIMES value(C) . { A = parser_node(2, Op_Mul, B, C); }
expression(A) ::= expression(B) DIVIDE value(C) . { A = parser_node(2, Op_Div, B, C); }
